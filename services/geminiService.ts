...
const flashcardSchema = {
    type: Type.OBJECT,
    properties: {
        topic: { type: Type.STRING, description: "The main topic of the flashcard (e.g., 'Cloud Storage')." },
        content: { type: Type.STRING, description: "The concise explanation or key points for the topic." },
    },
...
export const generateQuizQuestions = async (topic: string, count: number): Promise<QuizQuestion[]> => {
    try {
        ...
        throw new Error("Failed to generate quiz questions. The API may be unavailable or the request was malformed.");
    }
};
...
export const generateFlashcards = async (count: number): Promise<Flashcard[]> => {
    try {
        ...
        throw new Error("Failed to generate flashcards. The API may be unavailable or the request was malformed.");
    }
};
