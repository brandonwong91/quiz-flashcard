...
const QuizPage: React.FC = () => {
    ...
    if (isLoading) {
        return <div className="flex flex-col items-center justify-center h-96 gap-4">
            <LoadingSpinner />
            <p className="text-slate-600">Generating your quiz on {selectedTopic}...</p>
        </div>;
    }
    ...
    if (questions.length === 0) {
        return (
            <div className="text-center max-w-lg mx-auto">
                ...
                <p className="text-slate-600 mb-6">Select a topic to start a 5-question quiz generated by AI.</p>
                <div className="mb-6">
                    <label htmlFor="topic-select" className="block text-sm font-medium text-slate-700 mb-2">Choose a topic:</label>
                    <select
                        id="topic-select"
                        value={selectedTopic}
    ...
    if (isFinished) {
        ...
        return (
            <div className="text-center max-w-md mx-auto bg-white p-8 rounded-xl shadow-lg">
                ...
                <p className="text-slate-600 mb-6">You scored <span className="font-bold">{score}</span> out of <span className="font-bold">{questions.length}</span>.</p>
                ...
                <div className="flex gap-4">
                    <button onClick={restartQuiz} className="flex-1 bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors">
                        Try Again ({selectedTopic})
                    </button>
                     <button onClick={resetToTopicSelection} className="flex-1 bg-slate-200 text-slate-800 font-semibold py-2 px-4 rounded-lg hover:bg-slate-300 transition-colors">
                        Choose New Topic
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg">
                <div className="mb-4">
                    ...
                    <div className="w-full bg-slate-200 rounded-full h-2 mt-1">
                        <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${((currentQuestionIndex + 1) / questions.length) * 100}%` }}></div>
                    </div>
                </div>
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <span>{option}</span>
                                    {selectedAnswer !== null && isSelected && (isCorrect ? <CheckCircle className="text-green-600" /> : <XCircle className="text-red-600" />)}
                                    {selectedAnswer !== null && !isSelected && isCorrect && <CheckCircle className="text-green-600" />}
                                </div>
                            </button>
                        );
                    })}
